...
def main():
        ...
        if user_question:
            ...
            for document in context:
                with st.expander("관련 문서"):
                    ...
                    page_number = (document.metadata.get('page') or 0) + 1
                    button_key = f"link_{file_path}_{page_number}_{uuid.uuid4()}"
    ...
    with right_column:
        ...
        if page_number is not None and file_path:
            try:
                ...
                file_name = os.path.basename(file_path)  # 경로에서 파일 이름만 추출
                image_filename = f"{file_name}_page_{page_number}.png"
                ...
                if os.path.exists(image_path):
                    ...
                    st.warning(f"이미지 파일을 찾을 수 없습니다: {image_filename}")
